...
using NLog;
using BalanceChange = Miningcore.Api.Responses.BalanceChange;
using Block = Miningcore.Api.Responses.Block;
using MinerSettings = Miningcore.Api.Responses.MinerSettings;
using MinerStats = Miningcore.Api.Responses.MinerStats;
using Payment = Miningcore.Api.Responses.Payment;
using WorkerPerformanceStatsContainer = Miningcore.Api.Responses.WorkerPerformanceStatsContainer;
...
public class PoolApiController : ApiControllerBase
{
    ...
    private async Task<WorkerPerformanceStatsContainer[]> GetMinerPerformanceInternal(
    ...
    public async Task<MinerPerformanceStats[]> PagePoolMinersAsync(
        string poolId, [FromQuery] int page, [FromQuery] int pageSize = 15, [FromQuery] uint topMinersRange = 1)
    {
        var pool = GetPool(poolId);
        var ct = HttpContext.RequestAborted;
    ...
    public async Task<Block[]> PagePoolBlocksAsync(
    ...
    public async Task<PagedResultResponse<Block[]>> PagePoolBlocksV2Async(
        string poolId, [FromQuery] int page, [FromQuery] int pageSize = 15, [FromQuery] BlockStatus[] state = null)
    {
        var pool = GetPool(poolId);
        var ct = HttpContext.RequestAborted;
        ...
        var itemCount = await cf.Run(con => blocksRepo.GetPoolBlockCountAsync(con, poolId, ct));
        var pageCount = (uint) Math.Floor(itemCount / (double) pageSize);
        ...
        var response = new PagedResultResponse<Block[]>(blocks, itemCount, pageCount);
        return response;
    }

    [HttpGet("{poolId}/payments")]
    public async Task<Payment[]> PagePoolPaymentsAsync(
    ...
    public async Task<PagedResultResponse<Payment[]>> PagePoolPaymentsV2Async(
        string poolId, [FromQuery] int page, [FromQuery] int pageSize = 15)
    {
        var pool = GetPool(poolId);
        var ct = HttpContext.RequestAborted;

        var itemCount = await cf.Run(con => paymentsRepo.GetPaymentsCountAsync(con, poolId, null, ct));
        var pageCount = (uint) Math.Floor(itemCount / (double) pageSize);
        ...
        var response = new PagedResultResponse<Payment[]>(payments, itemCount, pageCount);
        return response;
    }

    [HttpGet("{poolId}/miners/{address}")]
    public async Task<MinerStats> GetMinerInfoAsync(
    ...
    public async Task<Block[]> PageMinerBlocksAsync(
    ...
    public async Task<PagedResultResponse<Block[]>> PageMinerBlocksV2Async(
        string poolId, string address, [FromQuery] int page, [FromQuery] int pageSize = 15, [FromQuery] BlockStatus[] state = null)
    {
        var pool = GetPool(poolId);
        var ct = HttpContext.RequestAborted;
        ...
        var itemCount = await cf.Run(con => blocksRepo.GetMinerBlockCountAsync(con, poolId, address, ct));
        var pageCount = (uint) Math.Floor(itemCount / (double) pageSize);
    ...
    public async Task<Payment[]> PageMinerPaymentsAsync(
    ...
    public async Task<PagedResultResponse<Payment[]>> PageMinerPaymentsV2Async(
        string poolId, string address, [FromQuery] int page, [FromQuery] int pageSize = 15)
    {
        var pool = GetPool(poolId);
        var ct = HttpContext.RequestAborted;
        ...
        var itemCount = await cf.Run(con => paymentsRepo.GetPaymentsCountAsync(con, poolId, address, ct));
        var pageCount = (uint) Math.Floor(itemCount / (double) pageSize);
    ...
    public async Task<BalanceChange[]> PageMinerBalanceChangesAsync(
        string poolId, string address, [FromQuery] int page, [FromQuery] int pageSize = 15)
    {
        var pool = GetPool(poolId);
        var ct = HttpContext.RequestAborted;
    ...
    public async Task<PagedResultResponse<BalanceChange[]>> PageMinerBalanceChangesV2Async(
        string poolId, string address, [FromQuery] int page, [FromQuery] int pageSize = 15)
    {
        var pool = GetPool(poolId);
        var ct = HttpContext.RequestAborted;
        ...
        var itemCount = await cf.Run(con => paymentsRepo.GetBalanceChangesCountAsync(con, poolId, address));
        var pageCount = (uint) Math.Floor(itemCount / (double) pageSize);
        ...
        var response = new PagedResultResponse<BalanceChange[]>(balanceChanges, itemCount, pageCount);
        return response;
    }
        ...
        string poolId, string address, [FromQuery] int page, [FromQuery] int pageSize = 15)
    {
        ...
        var itemCount = await cf.Run(con => paymentsRepo.GetMinerPaymentsByDayCountAsync(con, poolId, address));
        var pageCount = (uint) Math.Floor(itemCount / (double) pageSize);
    ...
    public async Task<WorkerPerformanceStatsContainer[]> GetMinerPerformanceAsync(
    ...
    public async Task<MinerSettings> GetMinerSettingsAsync(string poolId, string address)
    {
        ...
        return mapper.Map<MinerSettings>(result);
    }

    [HttpPost("{poolId}/miners/{address}/settings")]
    public async Task<MinerSettings> SetMinerSettingsAsync(string poolId, string address,
        [FromBody] UpdateMinerSettingsRequest request, CancellationToken ct)
    {
        var pool = GetPool(poolId);
