...
public static class BitcoinUtils
{
    ...
    {
        var bcash = BCash.Instance.GetNetwork(expectedNetwork.ChainName);
        var trashAddress = bcash.Parse<BCash.BTrashPubKeyAddress>(address);
    ...
    {
        var litecoin = Litecoin.Instance.GetNetwork(expectedNetwork.ChainName);
