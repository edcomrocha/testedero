...
public class ConcealJobManager : JobManagerBase<ConcealJob>
{
    ...
    {
        var response = await rpc.ExecuteAsync<SubmitResponse>(logger, ConcealCommands.SubmitBlock, CancellationToken.None, new[]
        {
            blobHex
        });
    ...
    public ConcealCoinTemplate Coin { get; private set; }
    ...
    protected virtual void SetupJobUpdates(CancellationToken ct)
    {
        ...
        if(extraPoolConfig?.BtStream == null)
        {
            ...
            if(zmq.Count > 0)
            {
                ...
                pollTimerRestart = blockSubmission.Merge(blockNotify.Select(_ => Unit.Default))
                    .Publish()
                    .RefCount();
