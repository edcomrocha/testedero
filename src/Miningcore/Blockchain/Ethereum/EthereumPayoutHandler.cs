    ...
    IPayoutHandler
{
    ...
    private async Task<DaemonResponses.Block[]> FetchBlocks(Dictionary<long, DaemonResponses.Block> blockCache, CancellationToken ct, params long[] blockHeights)
    {
        ...
        if(cacheMisses.Any())
        {
            var blockBatch = cacheMisses.Select(height => new RpcRequest(EC.GetBlockByNumber,
                new[]
                {
                    (object) height.ToStringHexWithPrefix(), true
                })).ToArray();
    ...
    internal static decimal GetBaseBlockReward(GethChainType chainType, ulong height)
    {
        switch(chainType)
        {
            ...
            case GethChainType.Mordor:
                ...
                var era = Math.Truncate(heightEraLength);
                var quotient = Convert.ToDecimal(Math.Pow(EthereumClassicConstants.DisinflationRateQuotient, era));
                var divisor = Convert.ToDecimal(Math.Pow(EthereumClassicConstants.DisinflationRateDivisor, era));
            ...
            case GethChainType.DogeEthereum:
                return DogeEthereumConstants.BaseRewardInitial;
    ...
    private async Task<decimal> GetTxRewardAsync(DaemonResponses.Block blockInfo, CancellationToken ct)
    {
        ...
        var batch = blockInfo.Transactions.Select(tx => new RpcRequest(EC.GetTxReceipt, new[]
            {
                tx.Hash
            }))
            .ToArray();
    ...
    internal static decimal GetUncleReward(GethChainType chainType, ulong uheight, ulong height)
    {
        ...
        switch(chainType)
        {
            ...
            case GethChainType.Mordor:
                double heightEraLength = height / EthereumClassicConstants.EraLength;
                var era = Math.Floor(heightEraLength);
                if(era == 0)
                {
    ...
    private async Task<string> PayoutAsync(Balance balance, CancellationToken ct)
    {
        ...
        if(extraPoolConfig?.ChainTypeOverride == "Ethereum" || extraPoolConfig?.ChainTypeOverride == "Main" || extraPoolConfig?.ChainTypeOverride == "Ubiq" || extraPoolConfig?.ChainTypeOverride == "MainPow" || extraPoolConfig?.ChainTypeOverride == "EtherOne")
        {
            ...
            {
                // get latest block
                var latestBlockResponse = await rpcClient.ExecuteAsync<DaemonResponses.Block>(logger, EC.GetBlockByNumber, ct, new[]
                {
                    (object) "latest", true
                });
        ...
        if(extraPoolConfig?.ChainTypeOverride == "DogeEthereum")
        {
            var requestDoge = new SendTransactionRequestDoge
            {
                From = poolConfig.Address,
                To = balance.Address,
                Value = amount.ToString("x").TrimStart('0'),
        ...
        // update db
        await PersistPaymentsAsync(new[]
        {
            balance
        }, txHash);
            ...
            for(var j = 0; j < blockInfos.Length; j++)
            {
                ...
                if(string.Equals(blockInfo.Miner, poolConfig.Address, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if(latestBlockHeight - block.BlockHeight >= EthereumConstants.MinConfimations)
                    {
                        ...
                        var totalDuplicateBlock = await cf.Run(con => blockRepo.GetPoolDuplicateBlockCountByPoolHeightNoTypeAndStatusAsync(con, poolConfig.Id, Convert.ToInt64(block.BlockHeight), new[]
                        {
                            block.Status
                        }));
                        ...
                        else
                        {
                            var blockHashResponse = await rpcClient.ExecuteAsync<DaemonResponses.Block>(logger, EC.GetBlockByNumber, ct,
                                new[]
                                {
                                    (object) block.BlockHeight.ToStringHexWithPrefix(), true
                                });
                            ...
                            if(string.Equals(blockMiner, poolConfig.Address, StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                if(extraPoolConfig?.ChainTypeOverride == "Ethereum" || extraPoolConfig?.ChainTypeOverride == "Main" || extraPoolConfig?.ChainTypeOverride == "MainPow" || extraPoolConfig?.ChainTypeOverride == "Ubiq" || extraPoolConfig?.ChainTypeOverride == "EtherOne" || extraPoolConfig?.ChainTypeOverride == "DogeEthereum" || extraPoolConfig?.ChainTypeOverride == "Hypra")
                ...
                foreach(var blockInfo2 in blockInfo2s)
                    ...
                    if(blockInfo2.Uncles.Length > 0)
                    {
                        ...
                        var uncleBatch = blockInfo2.Uncles.Select((x, index) => new RpcRequest(EC.GetUncleByBlockNumberAndIndex,
                                new[]
                                {
                                    blockInfo2.Height.Value.ToStringHexWithPrefix(), index.ToStringHexWithPrefix()
                                }))
                            .ToArray();
    ...
    public async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        if((networkType == EthereumNetworkType.Main || extraPoolConfig?.ChainTypeOverride == "Classic" || extraPoolConfig?.ChainTypeOverride == "Mordor" || networkType == EthereumNetworkType.MainPow || extraPoolConfig?.ChainTypeOverride == "Ubiq" || extraPoolConfig?.ChainTypeOverride == "EtherOne" || extraPoolConfig?.ChainTypeOverride == "DogeEthereum" || extraPoolConfig?.ChainTypeOverride == "OctaSpace" || extraPoolConfig?.ChainTypeOverride == "OctaSpaceTestnet" || extraPoolConfig?.ChainTypeOverride == "Hypra") &&
