...
public class EthereumJobManager : JobManagerBase<EthereumJob>
{
    ...
    private async Task<EthereumBlockTemplate> GetBlockTemplateAsync(CancellationToken ct)
    {
        var requests = new[]
        {
            new RpcRequest(EC.GetWork), new RpcRequest(EC.GetBlockByNumber, new[]
            {
                (object) "latest", true
            })
        };
        ...
        if(work.Length < 4)
        {
            ...
            work = work.Concat(new[]
            {
                (currentHeight + 1).ToStringHexWithPrefix()
            }).ToArray();
        }
    ...
    private async Task UpdateNetworkStatsAsync(CancellationToken ct)
    {
        try
        {
            var requests = new[]
            {
                new RpcRequest(EC.GetPeerCount), new RpcRequest(EC.GetBlockByNumber, new[]
                {
                    (object) "latest", true
                })
            };
            ...
            var sampleBlockResults = await rpc.ExecuteAsync<Block>(logger, EC.GetBlockByNumber, ct, new[]
            {
                (object) sampleBlockNumber.ToStringHexWithPrefix(), true
            });
    ...
    private async Task<bool> SubmitBlockAsync(Share share, string fullNonceHex, string headerHash, string mixHash)
    {
        ...
        {
            fullNonceHex, headerHash, mixHash
        });
    ...
    {
        var response = await rpc.ExecuteAsync<Block>(logger, EC.GetBlockByNumber, ct, new[]
        {
            (object) "latest", true
        });
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        var requests = new[]
        {
            new RpcRequest(EC.GetNetVersion), new RpcRequest(EC.GetAccounts), new RpcRequest(EC.GetCoinbase)
        };
    ...
    protected virtual async Task SetupJobUpdates(CancellationToken ct)
    {
        ...
        if(endpointExtra?.Item2?.PortWs.HasValue == true)
        {
            ...
            retry:
            // stream work updates
            var getWorkObs = rpc.WebsocketSubscribe(logger, ct, wsEndpointConfig, EC.Subscribe, new[]
                {
                    wsSubscription
                })
                .Publish()
