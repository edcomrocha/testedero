...
public class EthereumPool : PoolBase
{
        ...
        var data = new object[]
            {
                new object[]
                {
                    EthereumStratumMethods.MiningNotify, connection.ConnectionId, EthereumConstants.EthereumStratumVersion
                },
                context.ExtraNonce1
            }
            .ToArray();
    ...
    private async Task OnAuthorizeAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        if(context.IsAuthorized)
        {
            ...
            await connection.NotifyAsync(EthereumStratumMethods.SetDifficulty, new object[]
            {
                context.Difficulty
            });
