...
public class BeamJobManager : JobManagerBase<BeamJob>
{
    ...
    private readonly BeamHash solver = new();
        ...
        Task.Run(async () =>
        {
            using(cts)
            {
                ...
                try
                {
                    ...
                    client.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReceiveTimeout, 1);
                    client.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.NoDelay, true);
                    logger.Debug(() => $"[Submit Block] - Establishing socket connection with `{iPAddress.First()}:{port}`");
                    await client.ConnectAsync(ipEndPoint, cts.Token);
                    if(client.Connected)
    ...
    public object[] GetJobParamsForStratum()
    {
        ...
        return job?.GetJobParamsForStratum() ?? Empty<object>();
    }
    ...
    protected override void ConfigureDaemons()
    {
        ...
        explorerRestClient = new SimpleRestClient(httpClientFactory, "http://" + explorerDaemonEndpoints.First().Host + ":" + explorerDaemonEndpoints.First().Port + "/");
        logger.Debug(() => $"`beam-node-explorer` URL: http://{explorerDaemonEndpoints.First().Host}:{explorerDaemonEndpoints.First().Port.ToString()}/");
