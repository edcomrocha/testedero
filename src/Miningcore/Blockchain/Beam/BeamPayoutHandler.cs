    ...
    IPayoutHandler
{
    ...
    internal static decimal GetBaseBlockReward(ulong height)
    {
        ...
        if(height > BeamConstants.EmisssionThirdEpochHeight)
        {
            ...
            var quotient = Convert.ToDecimal(Math.Pow(BeamConstants.DisinflationRateQuotient, era));
            var divisor = Convert.ToDecimal(Math.Pow(BeamConstants.DisinflationRateDivisor, era));
            return BeamConstants.EmisssionSecondEpochBlockReward * quotient / divisor;
        }

        if(height > BeamConstants.EmisssionFirstEpochHeight)
    ...
    public virtual async Task ConfigureAsync(ClusterConfig cc, PoolConfig pc, CancellationToken ct)
    {
        ...
        restClient = new SimpleRestClient(httpClientFactory, "http://" + daemonEndpoints.First().Host + ":" + daemonEndpoints.First().Port + "/");
    ...
    public async Task<Block[]> ClassifyBlocksAsync(IMiningPool pool, Block[] blocks, CancellationToken ct)
    {
        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            for(var j = 0; j < page.Length; j++)
            {
                ...
                var IsOrphaned = !string.IsNullOrEmpty(block.Hash) ? !string.IsNullOrEmpty(rpcResult.Response?.Pow) && rpcResult.Response?.Pow.Contains(block.Hash) == false : !string.IsNullOrEmpty(rpcResult.Response?.BlockHash) && rpcResult.Response?.BlockHash == block.TransactionConfirmationData;
