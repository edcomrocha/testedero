    ...
    IPayoutHandler
{
    ...
    private async Task UnlockWallet(CancellationToken ct)
    {
        ...
        await Guard(() => alephiumClient.NameUnlockAsync(extraPoolPaymentProcessingConfig.WalletName, new WalletUnlock
        {
    ...
    public virtual async Task<Block[]> ClassifyBlocksAsync(IMiningPool pool, Block[] blocks, CancellationToken ct)
    {
        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            for(var j = 0; j < page.Length; j++)
            {
                ...
                // get block info
                var blockInfo = await Guard(() => alephiumClient.HashAsync(block.Hash, ct),
                    ex => logger.Debug(ex));
                ...
                if(extraPoolPaymentProcessingConfig?.BlockRewardsLockTime == null)
                {
                    ...
                    var totalTransactionsLockTime = 0;
                    ...
                    foreach(var unsignedLockTimeFixedOutputs in blockTransactionLockTime.Unsigned.FixedOutputs)
                        ...
                        {
                            transactionsLockTime += unsignedLockTimeFixedOutputs.LockTime;
                ...
                var isBlockInMainChain = await Guard(() => alephiumClient.GetBlockflowIsBlockInMainChainAsync(block.Hash, ct),
                    ex => logger.Debug(ex));
    ...
    public virtual async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        for(var j = 0; j < groupingAmounts.Length; j++)
        {
            ...
            if(initialTotalAddresses > 0)
            {
                ...
                NotifyPayoutSuccess(poolConfig.Id, successBalances, new[]
                {
                    txSubmit.TxId
                }, estimatedGasAmount * AlephiumConstants.DefaultGasPrice / AlephiumConstants.SmallestUnit);
            }
        }
