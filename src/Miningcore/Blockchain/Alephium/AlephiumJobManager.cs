...
public class AlephiumJobManager : JobManagerBase<AlephiumJob>
{
        ...
        JsonSerializerSettings payloadJsonSerializerSettings = null)
    {
        return Observable.Defer(() => Observable.Create<AlephiumBlockTemplate[]>(obs =>
        {
            ...
            Task.Run(async () =>
            {
                ...
                {
                    socket_connexion:
                    var receiveBuffer = new byte[socketJobMessageBufferSize];

                    try
                    {
                        var port = extraDaemonEndpoint.MinerApiPort;
                        var iPAddress = await Dns.GetHostAddressesAsync(endPoint.Host, AddressFamily.InterNetwork, cts.Token);
                        var ipEndPoint = new IPEndPoint(iPAddress.First(), port);
                        ...
                        logger.Debug(() => $"Establishing socket connection with `{iPAddress.First()}:{port}`");
    ...
    {
        var tmpByte = reader.ReadBytes(count);
        Reverse(tmpByte);
    ...
    private async Task UpdateNetworkStatsAsync(CancellationToken ct)
    {
        try
        {
            ...
            BlockchainStats.NetworkDifficulty = infoDifficulty.Difficulty;
        ...
        JsonSerializerSettings payloadJsonSerializerSettings = null)
    {
        ...
        {
            var port = extraDaemonEndpoint.MinerApiPort;
            var iPAddress = await Dns.GetHostAddressesAsync(endPoint.Host, AddressFamily.InterNetwork, ct);
            var ipEndPoint = new IPEndPoint(iPAddress.First(), port);
            using Socket client = new(SocketType.Stream, ProtocolType.Tcp);
            //client.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.KeepAlive, true);
            client.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendTimeout, 1);
    ...
    public AlephiumCoinTemplate Coin { get; private set; }
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        ...
        if(clusterConfig.PaymentProcessing?.Enabled == true && poolConfig.PaymentProcessing?.Enabled == true)
        {
            ...
            if(validityWalletName.Locked)
            {
                ...
                await Guard(() => rpc.NameUnlockAsync(extraPoolPaymentProcessingConfig.WalletName, new WalletUnlock
                    {
