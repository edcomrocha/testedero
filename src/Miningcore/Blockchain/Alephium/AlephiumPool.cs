...
public class AlephiumPool : PoolBase
{
    ...
    private async Task SendJob(StratumConnection connection, AlephiumWorkerContext context, AlephiumJobParams jobParams)
    {
        ...
        // send difficulty
        await connection.NotifyAsync(AlephiumStratumMethods.SetDifficulty, new object[]
        {
            context.Difficulty
        });

        // send job
        await connection.NotifyAsync(AlephiumStratumMethods.MiningNotify, new object[]
        {
            jobParamsActual
        });
    }
