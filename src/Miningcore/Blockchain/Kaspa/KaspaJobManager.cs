...
public class KaspaJobManager : JobManagerBase<KaspaJob>
{
        ...
        try
        {
            ...
            await foreach(var infoHashrate in stream.ResponseStream.ReadAllAsync())
            {
                if(string.IsNullOrEmpty(infoHashrate.EstimateNetworkHashesPerSecondResponse.Error?.Message))
                    BlockchainStats.NetworkHashrate = infoHashrate.EstimateNetworkHashesPerSecondResponse.NetworkHashesPerSecond;
    ...
    public object[] GetSubscriberData(StratumConnection worker)
    {
        ...
        {
            context.ExtraNonce1, KaspaConstants.ExtranoncePlaceHolderLength - extraNonce1Size
        };
    ...
    public bool ValidateIsBzMiner(string userAgent)
    {
        ...
        // Find matches
        var matchesUserAgentBzMiner = KaspaConstants.RegexUserAgentBzMiner.Matches(userAgent);
    ...
    public bool ValidateIsGodMiner(string userAgent)
    {
        ...
        // Find matches
        var matchesUserAgentGodMiner = KaspaConstants.RegexUserAgentGodMiner.Matches(userAgent);
    ...
    public bool ValidateIsIceRiverMiner(string userAgent)
    {
        ...
        // Find matches
        var matchesUserAgentIceRiverMiner = KaspaConstants.RegexUserAgentIceRiverMiner.Matches(userAgent);
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        ...
        await foreach(var info in stream.ResponseStream.ReadAllAsync())
        {
            ...
            extraData = info.GetInfoResponse.ServerVersion + (!string.IsNullOrEmpty(extraData) ? "." + extraData : "");
            break;
        }
        await stream.RequestStream.CompleteAsync();
    ...
    protected override async Task EnsureDaemonsSynchedAsync(CancellationToken ct)
    {
        ...
        do
        {
            ...
            await foreach(var info in stream.ResponseStream.ReadAllAsync())
            {
                ...
                isSynched = info.GetInfoResponse.IsSynced && info.GetInfoResponse.IsUtxoIndexed;
                break;
            }
            await stream.RequestStream.CompleteAsync();
