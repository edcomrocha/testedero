...
public class KaspaPool : PoolBase
{
    ...
    private async Task SendJob(StratumConnection connection, KaspaWorkerContext context, object[] jobParams)
    {
        ...
        if(context.IsLargeJob)
            jobParamsActual = new[]
            {
                jobParams[0], jobParams[1]
            };
        else
            jobParamsActual = new[]
            {
                jobParams[0], jobParams[2], jobParams[3]
            };

        // send difficulty
        await connection.NotifyAsync(KaspaStratumMethods.SetDifficulty, new object[]
        {
            context.Difficulty
        });
        ...
        try
        {
            switch(request.Method)
            {
                ...
                case KaspaStratumMethods.ExtraNonceSubscribe:
                    ...
                    var data = new object[]
                    {
                        context.ExtraNonce1, KaspaConstants.ExtranoncePlaceHolderLength - manager.GetExtraNonce1Size()
                    };
