...
public class NexaJobManager : BitcoinJobManagerBase<NexaJob>
{
    ...
    public object[] GetSubscriberData(StratumConnection worker)
    {
        ...
        {
            context.ExtraNonce1, BitcoinConstants.ExtranoncePlaceHolderLength - ExtranonceBytes
        };
    ...
    {
        var rawSubmitRes = await rpc.ExecuteAsync<object>(logger, NexaCommands.SubmitMiningSolution, ct, args);
        ...
        var blockResult = await rpc.ExecuteAsync<Block>(logger, BitcoinCommands.GetBlock, ct, new object[]
        {
            submitResult?.Hash
        });
