        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            for(var j = 0; j < page.Length; j++)
            {
                ...
                if(blockHeader.Depth >= CryptonoteConstants.PayoutMinBlockConfirmations)
                {
                    ...
                    {
                        case "ZEPH":
                            var osirisHardForkBlock = ZephyrConstants.OsirisHardForkBlockMainnet;
                            var miningReward = ZephyrConstants.MiningRewardInitial;
                            var reserveReward = ZephyrConstants.ReserveRewardInitial;
