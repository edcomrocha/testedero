...
public class ProgpowPool : PoolBase
{
    ...
    protected virtual async Task OnSubscribeAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        var data = new object[]
            {
                new object[]
                {
                    new object[]
                    {
                        ProgpowStratumMethods.SetDifficulty, connection.ConnectionId
                    },
                    new object[]
                    {
                        ProgpowStratumMethods.MiningNotify, connection.ConnectionId
                    }
                }
            }
        ...
        // send intial update
        await connection.NotifyAsync(ProgpowStratumMethods.SetDifficulty, new object[]
        {
    ...
    private object CreateWorkerJob(StratumConnection connection, bool update)
    {
        ...
        {
            job.Id, headerHash, job.SeedHash, createEncodeTarget(context.Difficulty), update, job.Height, job.Bits
        };
    ...
    protected override async Task OnVarDiffUpdateAsync(StratumConnection connection, double newDiff, CancellationToken ct)
    {
        ...
        if(connection.Context.ApplyPendingDifficulty())
        {
            ...
            await connection.NotifyAsync(ProgpowStratumMethods.SetDifficulty, new object[]
            {
                createEncodeTarget(connection.Context.Difficulty)
            });
            await connection.NotifyAsync(ProgpowStratumMethods.MiningNotify, minerJobParams);
        }
    }
