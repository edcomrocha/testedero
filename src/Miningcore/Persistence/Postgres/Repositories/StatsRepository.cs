...
public class StatsRepository : IStatsRepository
{
    ...
    public async Task<PoolStats> GetLastPoolStatsAsync(IDbConnection con, string poolId, CancellationToken ct)
    {
        ...
        var entity = await con.QuerySingleOrDefaultAsync<Entities.PoolStats>(new CommandDefinition(query, new
        {
            poolId
    ...
    public Task<decimal> GetTotalPoolPaymentsAsync(IDbConnection con, string poolId, CancellationToken ct)
    {
        ...
        return con.ExecuteScalarAsync<decimal>(new CommandDefinition(query, new
        {
            poolId
        }, cancellationToken: ct));
    }
        ...
        SampleInterval interval, DateTime start, DateTime end, CancellationToken ct)
    {
        ...
        return (await con.QueryAsync<Entities.PoolStats>(new CommandDefinition(query, new
            {
                poolId,
                start,
                end
    ...
    public async Task<MinerStats> GetMinerStatsAsync(IDbConnection con, IDbTransaction tx, string poolId, string miner, CancellationToken ct)
    {
        ...
        var result = await con.QuerySingleOrDefaultAsync<MinerStats>(new CommandDefinition(query, new
        {
            poolId,
            miner
        ...
        if(result != null)
        {
            ...
            result.LastPayment = await con.QuerySingleOrDefaultAsync<Payment>(new CommandDefinition(query, new
            {
                poolId,
                miner
            }, tx, cancellationToken: ct));
            ...
            var lastUpdate = await con.QuerySingleOrDefaultAsync<DateTime?>(new CommandDefinition(query, new
            {
                poolId,
                miner
            }, tx, cancellationToken: ct));
    ...
    public async Task<MinerWorkerHashrate[]> GetPoolMinerWorkerHashratesAsync(IDbConnection con, string poolId, CancellationToken ct)
    {
        ...
        return (await con.QueryAsync<MinerWorkerHashrate>(new CommandDefinition(query, new
            {
                poolId
            }, cancellationToken: ct)))
            .ToArray();
