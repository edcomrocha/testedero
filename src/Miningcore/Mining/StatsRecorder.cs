...
public class StatsRecorder : BackgroundService
{
        ...
        IStatsRepository statsRepo)
    {
        ...
        hashrateCalculationWindow = TimeSpan.FromMinutes(clusterConfig.Statistics?.HashrateCalculationWindow ?? 2);
        cleanupDays = TimeSpan.FromDays(clusterConfig.Statistics?.CleanupDays ?? 15);
    ...
    private async Task UpdatePoolHashratesAsync(CancellationToken ct)
    {
        ...
        foreach(var poolId in pools.Keys)
        {
            ...
            foreach(var minerHashes in byMiner)
            {
                ...
                messageBus.NotifyHashrateUpdated(pool.Config.Id, minerTotalHashrate, stats.Miner);
