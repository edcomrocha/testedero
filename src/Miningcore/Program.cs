...
using NexaPow = Miningcore.Crypto.Hashing.Algorithms.NexaPow;
...
");
    ...
    private static async Task PreFlightChecks(IServiceProvider services)
    {
        ...
        // Configure Ethhash
        Cache.messageBus = messageBus;

        // Configure Etchash
        Crypto.Hashing.Ethash.Etchash.Cache.messageBus = messageBus;

        // Configure Ethashb3
        Crypto.Hashing.Ethash.Ethashb3.Cache.messageBus = messageBus;

        // Configure Ubqhash
        Crypto.Hashing.Ethash.Ubqhash.Cache.messageBus = messageBus;
        ...
        // Configure NexaPow
        NexaPow.messageBus = messageBus;

        // Configure BeamHash
        BeamHash.messageBus = messageBus;

        // Configure FiroPow
        Crypto.Hashing.Progpow.Firopow.Cache.messageBus = messageBus;

        // Configure Kawpow
        Crypto.Hashing.Progpow.Kawpow.Cache.messageBus = messageBus;
    }
    ...
    private static async Task ConfigurePostgresCompatibilityOptions(IServiceProvider services)
    {
        ...
        var enableLegacyTimestampBehavior = false;
    ...
    private static async Task<string> GetPostgresColumnType(IConnectionFactory cf, string table, string column)
    {
        ...
        return await cf.Run(async con => await con.ExecuteScalarAsync<string>(query, new
        {
            table,
            column
        }));
