...
public class StratumConnectionBenchmarks : TestBase
{
    ...
    public void Setup()
    {
        ...
        connection = new StratumConnection(logger, rmsm, clock, ConnectionId, false);
        wrapper = new PrivateObject(connection);
    }

    private Task OnPlaceholderRequestAsync(StratumConnection con, JsonRpcRequest request, CancellationToken ct)
    {
